 ...
 */
public class MessageRunActor extends AbstractActor {

    @Inject
    public MessageRunActor(ItemService itemService, WSClient ws, Configuration configuration) {
            ...
            if (versionVoList != null && versionVoList.size() > 0) {
                ...
                F.Promise<File> filePromise = ws.url(configuration.getString("zip.download.url") + URLEncoder.encode(versionVo.getDownloadLink(), "UTF-8")).get().map(response -> {
                    ...
                    try {
                        ...
                        String zipPath = configuration.getString("message.zip.path");

                        rmShell(zipPath,projectName);

                        final File file = new File(zipPath);

                        Logger.error("message.zip.path文件为: " + file.getPath());
